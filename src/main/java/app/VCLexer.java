...
class VCLexer {
    ...
    public void addToken(String token, String typeToken) {
        String spelling = token.trim();
        int pos = column + 1;
        String key = "Kind = " + state
                + " [" + typeToken + "], "
                +  "spelling = \"" + spelling + "\", "
                + "position = " + (numOfLine + 1) + "(" + pos + ")"
                + ".." + (numOfLine + 1) + "(" + (pos + spelling.length() - 1) + ")";
        ;
        characters.add(token.trim());
    ...
    public void writeVerbose(String filename) {
        String verbose = filename + "verbose.vctok";
        String vc = filename + "vctok";

        createFile(verbose);
        createFile(vc);

        writeFile(verbose, tokens);
        writeFile(vc, characters);
    }
    ...
    public static void main(String argv[]) {
        if (argv.length == 0) {
            ...
            for (int i = firstFilePos; i < argv.length; i++) {
                ...
                scanner.printTokens();
                scanner.writeVerbose(argv[i].substring(0, argv[i].length() - 2));
            }
        }
    }
}
